/* VENDORED from IDF 5.5.4 components/esp_lcd/dsi/esp_lcd_panel_dpi.c (v0.4.2).
 * Shadows the archive copy at link time (object files beat archive members).
 * Three patches against DSI underrun tail latency -- the source of the visible
 * flashes (bridge underflow substitute pixels; see panel.cpp + memory notes):
 *  1. The per-frame DMA re-arm ISR runs at PRIORITY 3 on a dedicated vector
 *     (stock: shared LOWMED -- a delayed re-arm starts the frame starved;
 *     externally confirmed fix for this exact signature).
 *  2. Bridge FIFO empty threshold 1024-256 -> 1024-64: refill starts earlier.
 *  3. DST burst 256 -> 64 items: shorter unpreemptible bridge-write windows
 *     (invariant kept: depth - threshold >= one dst burst).
 * Update alongside IDF platform bumps. */
/*
...
bool IRAM_ATTR mipi_dsi_dma_trans_done_cb(dw_gdma_channel_handle_t chan, const dw_gdma_trans_done_event_data_t *event_data, void *user_data)
{
...
void IRAM_ATTR mipi_dsi_bridge_isr_handler(void *args)
{
...
static esp_err_t dpi_panel_create_dma_link(esp_lcd_dpi_panel_t *dpi_panel)
{
    ...
    dw_gdma_channel_alloc_config_t dma_alloc_config = {
        .intr_priority = 3,                       // [PATCH 1] dedicated level-3 re-arm vector
        .src = {
...
esp_err_t esp_lcd_new_panel_dpi(esp_lcd_dsi_bus_handle_t bus, const esp_lcd_dpi_panel_config_t *panel_config, esp_lcd_panel_handle_t *ret_panel)
{
    ...
    mipi_dsi_brg_ll_set_empty_threshold(hal->bridge, 1024 - 64);   // [PATCH 2] refill sooner
    // enable DSI bridge
...
static esp_err_t dpi_panel_init(esp_lcd_panel_t *panel)
{
    ...
    dw_gdma_block_transfer_config_t dma_transfer_config = {
        ...
        .dst = {
            ...
            .burst_items = DW_GDMA_BURST_ITEMS_64,   // [PATCH 3] depth-threshold >= one burst
            .burst_len = 16,
            .width = DW_GDMA_TRANS_WIDTH_64,
        },
