...
// sdcard.cpp -- see sdcard.h. SD_MMC 4-bit mode on the Waveshare-documented pins.
...
// on-board LDO channel 4 -- no LDO, no card. Slot selection and LDO power are
// BOTH handled by the core's SD_MMC via build flags (platformio.ini):
// BOARD_HAS_SDMMC + BOARD_SDMMC_SLOT=0 + BOARD_SDMMC_POWER_CHANNEL=4. Without
// the slot flag the core hardcodes SLOT 1 -- which the C6 radio (esp_hosted
// SDIO) owns -- and the mount can never succeed. sdInit() must keep running
// BEFORE any radio init (main.cpp order) so a failed mount's host cleanup can
// never tear down a live esp_hosted slot.
...
void sdInit() {
  ...
  // With BOARD_SDMMC_SLOT=0 this is a pure validation call: the core checks the
  // pins against SLOT 0's IOMUX set, so a wiring-assumption drift fails loudly here.
